    ...
    @if ($posts->count())
        <div
            ...
            <div class="flex flex-col  p-4 leading-normal">
                <h5 class="mb-2 text-2xl leading-snug font-bold tracking-tight text-gray-900 dark:text-white"><a
                        class="hover:underline" href="/news/{{ $posts[0]->slug }}">{{ $posts[0]->title }}</a>
                </h5>
                <small>
                    <p class="font-normal mb-2 text-gray-500 text-sm">By: <a class="text-blue-500 hover:underline"
                        ...
                        in <a class="text-blue-500 hover:underline"
                            href="{{ route('categories.posts', ['category' => $posts[0]->category->slug]) }}">
        ...
        <div class="max-md:flex-col flex md:gap-5">
            ...
            <div class="flex flex-col">
                <a href="/news/{{ $posts[0]->slug }}">
                    <h1
                        class="text-lg leading-snug line-clamp-3 tracking-tight md:line-clamp-2 md:text-3xl font-bold group-hover:text-blue-500 transition-all duration-500 ease-in-out">
                        {{ $posts[0]->title }}</h1>
                </a>
                <small>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 pt-32">
            @foreach ($categories as $category)
                <div class="flex flex-col gap-5">
                    <div class="flex gap-2 items-center">
                        <h1 class="text-4xl md:text-3xl font-bold">{{ $category->name }}</h1>
                    ...
                    @foreach ($categoryPosts[$category->id] as $post)
                        <div class="flex flex-col gap-5">
                            <a class="group" href="">
                                <div class="">
                                    <img class="md:w-full max-w-xs aspect-video mb-4 object-cover overflow-hidden bg-black/30 rounded-2xl "
                                        src="{{ asset('storage/' . $post->image) }}" alt="">
                                </div>
                                <div class="flex flex-col">
                                    <h5
                                        ...
                                        {{ $post->title }}</h5>
                                    <small>
                                        <p class="font-normal mb-2 text-gray-500 text-sm">By: <a
                                                class="text-blue-500 hover:underline"
                                                href="{{ route('users.posts', ['user' => $posts[0]->author->slug]) }}">
                                                {{ $posts[0]->author->username }}
                                            </a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5 py-5">
                @foreach ($categories as $category)
                    <a href="/category/{{ $category->slug }}"
                        ...
                        <h3 class="text-sm md:text-2xl">{{ $category->name }}</h3>
